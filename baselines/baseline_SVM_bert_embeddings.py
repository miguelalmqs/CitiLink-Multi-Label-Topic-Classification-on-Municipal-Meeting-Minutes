"""
SVM with BERTimbau Embeddings for Multi-Label Text Classification
of Portuguese Municipal Minutes Discussion Subjects

This module implements a Support Vector Machine classifier using BERTimbau (large)
embeddings for Portuguese municipal text classification.
The model uses dynamic threshold optimization per label to handle class imbalance effectively.

Key Features:
    - BERTimbau-large embeddings (neuralmind/bert-large-portuguese-cased)
...
MODEL_NAME = "SVM_BERTimbau_large_DynamicThreshold"

# BERTimbau model configuration
...
def clean_text(text):
    """
    Basic text cleaning: lowercase and whitespace normalization.
...
def get_bertimbau_embeddings(texts, tokenizer, model, batch_size=16, max_length=512):
    ...
    This function processes texts in batches and computes mean-pooled
    representations from the last hidden layer of BERTimbau.
    ...
    Returns:
        numpy.ndarray: Matrix of embeddings (n_samples, embedding_dim)
    """
    all_embeddings = []
...
def main():
    ...
    """
    print("\n" + "="*70)
    print("SVM WITH BERTIMBAU EMBEDDINGS - MULTI-LABEL CLASSIFICATION")
    print("="*70 + "\n")
    ...
    dump(clf, "svm_bertimbau_multilabel_model.joblib")
    print("  ✓ SVM model")

    np.save("optimal_thresholds_bertimbau.npy", np.array(optimal_thresholds))
    print("  ✓ Optimal thresholds")

    dump(mlb, "mlb_encoder_bertimbau.joblib")
